...
type LoadRestaurantMenuOptions = {
  categoryName?: string;
};
...
export async function loadRestaurantMenu(
  slug: string,
  options: LoadRestaurantMenuOptions = {}
) {
  ...
  const targetCategory = normalizeCategoryName(options.categoryName);
  ...
  const categories = Array.isArray(activeMenu?.menu_categories)
    ? activeMenu!.menu_categories
        ...
        .filter((category) => {
          ...
          return normalizeCategoryName(category.name) === targetCategory;
        })
        .sort(
