...
function buildStatusMessage(args: {
  ...
  if (status === "ready") {
    return `${restaurantName}: Your order ${orderNumber} is ready for pickup.`;
  }

  if (status === "completed") {
    return `${restaurantName}: Your order ${orderNumber} has been completed. Thank you.`;
  }
...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const { data: order, error: orderError } = await supabase
      .schema("food_ordering")
      .from("orders")
      .select("id, status, restaurant_id, order_number, customer_id")
      .eq("id", orderId)
